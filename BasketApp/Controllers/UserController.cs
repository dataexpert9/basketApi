            ...
            try
            {
                ...
                using (BasketContext ctx = new BasketContext())
                {
                    ...
                    if (model.SignInType == (int)RoleTypes.User)
                    {
                        ...
                        userModel = ctx.Users.Include(x => x.UserAddresses).Include(x => x.PaymentCards).FirstOrDefault(x => x.Email == model.Email && x.Password == hashedPassword && x.IsDeleted == false);
                    ...
                    {
                        delivererModel = ctx.DeliveryMen.Include(x => x.DelivererAddresses).FirstOrDefault(x => x.Email == model.Email && x.Password == CryptoHelper.Hash(model.Password));
                        if (delivererModel != null)
                        {
                            if (delivererModel.IsDeleted)
                            {
                                return Content(HttpStatusCode.OK, new CustomResponse<Error>
                                {
                                    Message = "Forbidden",
                                    StatusCode = (int)HttpStatusCode.Forbidden,
                                    Result = new Error { ErrorMessage = "Admin did not mark your account as active yet." }
                                });
                            }
                            await delivererModel.GenerateToken(Request);
                            BasketSettings.LoadSettings();
                            delivererModel.BasketSettings = new Settings
                            {
                                Id = BasketSettings.Id,
                                MinimumOrderPrice = BasketSettings.MinimumOrderPrice,
                                Currency = BasketSettings.Currency,
        ...
        public async Task<IHttpActionResult> Register()
        {
            try
            {
                ...
                using (BasketContext ctx = new BasketContext())
                {
                    ...
                    {
                        if (ctx.DeliveryMen.Any(x => x.Email == model.Email))
                        {
                            return Content(HttpStatusCode.OK, new CustomResponse<Error>
                            {
                                Message = "Conflict",
                    ...
                    if (model.SignInType == (int)RoleTypes.User)
                    {
                        ...
                        //if (String.IsNullOrEmpty(newFullPath) == false)
                        //    userModel.ProfilePictureUrl = Utility.BaseUrl + ConfigurationManager.AppSettings["UserImageFolderPath"] + Path.GetFileName(newFullPath);
                    ...
                    else
                    {
                        if (ctx.DeliveryMen.Any(x => x.Email == model.Email))
                        {
                            return Content(HttpStatusCode.OK, new CustomResponse<Error>
                            {
                                Message = "Conflict",
                                StatusCode = (int)HttpStatusCode.Conflict,
                                Result = new Error { ErrorMessage = "User with email already exist" }
                            });
                        }
                        else
                        {
                            delivererModel = new DeliveryMan
                            {
                                Email = model.Email,
                                Password = CryptoHelper.Hash(model.Password),
                                FullName = model.FullName,
                                ...
                                Phone = model.PhoneNumber,
                                IsNotificationsOn = true,
                                IsDeleted = true // By default, deliverer is blocked
                            };

                            //if (String.IsNullOrEmpty(newFullPath) == false)
                            //    delivererModel.ProfilePictureUrl = Utility.BaseUrl + ConfigurationManager.AppSettings["DelivererImageFolderPath"] + Path.GetFileName(newFullPath);

                            ctx.DeliveryMen.Add(delivererModel);
                            ctx.SaveChanges();
                            if (httpRequest.Files.Count > 0)
                            {
                            ...
                            {
                                Title = "New Deliverer Registered!",
                                Text = "A new deliverer has been registered",
        ...
        public async Task<IHttpActionResult> ChangePassword(SetPasswordBindingModel model)
        {
            try
            {
                ...
                using (BasketContext ctx = new BasketContext())
                {
                    ...
                    {
                        var deliveryMan = ctx.DeliveryMen.FirstOrDefault(x => x.Email == userEmail && x.Password == CryptoHelper.Hash(model.OldPassword));
                        if (deliveryMan != null)
                        {
